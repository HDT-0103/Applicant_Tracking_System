    ...
    <div>
      ...
      <div className="flex gap-2 mb-3">
        <Input
          ...
          className="h-9 text-sm border-[rgba(15,17,23,0.15)] focus:border-[#4f46e5] focus:ring-2 focus:ring-[#4f46e5]/20"
        />
        <button
          type="button"
          onClick={commit}
  ...
  return (
    <div>
      ...
      <Textarea
        ...
        placeholder={placeholder}
        className="rounded-t-none border-[rgba(15,17,23,0.15)] hover:border-[rgba(15,17,23,0.25)] focus:ring-2 focus:ring-[#4f46e5]/20 focus:border-[#4f46e5] text-sm resize-none transition-all outline-none font-mono text-[13px] leading-relaxed"
      />
    </div>
  );
}
...
function AdminNav({ onGoToPortal }: { onGoToPortal: () => void }) {
  const navItems = [
    { icon: LayoutTemplate, label: "Job Postings", active: true },
    ...
    { icon: Briefcase, label: "Interviews", active: false },
  ];

  return (
    <nav className="w-[220px] shrink-0 bg-white border-r border-border flex flex-col overflow-y-auto">
      {/* Brand */}
      <div className="px-5 py-5 border-b border-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground leading-none">GeoComply</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">HR Admin</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-0.5 px-3 py-4 flex-1">
        <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground px-2 mb-2">Recruitment</p>
        {navItems.map(({ icon: Icon, label, active }) => (
          <button
            ...
            className={cn(
              ...
              active
                ? "bg-[#4f46e5]/8 text-[#4f46e5] font-medium"
                : "text-muted-foreground hover:bg-[rgba(15,17,23,0.04)] hover:text-foreground",
            )}
          >
            <Icon className={cn("w-4 h-4 shrink-0", active && "text-[#4f46e5]")} />
            {label}
            {active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#4f46e5]" />}
          </button>
        ))}
      </div>
      ...
      <div className="px-3 pb-5">
        <button
          type="button"
          onClick={onGoToPortal}
          className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg border border-[#4f46e5]/20 bg-[#4f46e5]/5 text-[#4f46e5] text-sm font-medium hover:bg-[#4f46e5]/10 transition-all"
        >
...
export default function HRDashboard({ onGoToPortal }: HRDashboardProps) {
  ...
  const inputCls = "h-10 text-sm border-[rgba(15,17,23,0.15)] hover:border-[rgba(15,17,23,0.25)] focus:border-[#4f46e5] focus:ring-2 focus:ring-[#4f46e5]/20 transition-all outline-none";
  const selectCls = "h-10 text-sm border-[rgba(15,17,23,0.15)]";
  ...
  return (
    ...
    >
      <AdminNav onGoToPortal={onGoToPortal} />

      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">

        {/* ── Top Header ────────────────────────────────────────────────── */}
        <header className="bg-white border-b border-border px-8 py-0 h-14 flex items-center gap-4 shrink-0 z-10">
          {/* Breadcrumb */}
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
            <span className="hover:text-foreground cursor-pointer transition-colors">Job Postings</span>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-foreground font-medium">Create New Position</span>
          </div>
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => { setSaveStatus("saving"); setTimeout(() => setSaveStatus("saved"), 1000); }}
              className="gap-1.5 border-[rgba(15,17,23,0.15)] text-foreground hover:bg-[#f4f5f7]"
            >
            ...
            <Button
              ...
              className="gap-1.5 bg-[#4f46e5] hover:bg-[#4338ca] text-white shadow-sm shadow-[#4f46e5]/20"
            >
        ...
        {/* ── Page Title Bar ─────────────────────────────────────────────── */}
        <div className="bg-white border-b border-border px-8 py-4 flex items-center gap-3 shrink-0">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto min-w-0">
            <div className="max-w-[760px] mx-auto px-8 py-7 flex flex-col gap-7">
              ...
              <div className="bg-white rounded-xl border border-border shadow-sm p-6">
                ...
                {/* Job Title — full width, prominent */}
                <div className="mb-5">
                  <Label htmlFor="jobTitle" className="text-sm font-medium text-foreground mb-1.5 flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <Select value={jd.department} onValueChange={(v) => set("department", v)}>
                      <SelectTrigger className={selectCls}>
                        <SelectValue placeholder="Select department…" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="engineering">Technology – Engineering</SelectItem>
                        <SelectItem value="search">Search & Ranking</SelectItem>
                  ...
                  <div>
                    ...
                    <Select value={jd.location} onValueChange={(v) => set("location", v)}>
                      <SelectTrigger className={selectCls}>
                        <SelectValue placeholder="Select location…" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="hcmc-onsite">Ho Chi Minh / On-site</SelectItem>
                        <SelectItem value="hanoi-onsite">Hanoi / On-site</SelectItem>
                        <SelectItem value="eu-remote">EU / Remote</SelectItem>
                        <SelectItem value="us-remote">US / Remote</SelectItem>
                        <SelectItem value="apac-remote">APAC / Remote</SelectItem>
                        <SelectItem value="global-remote">Global / Fully Remote</SelectItem>
                        <SelectItem value="vancouver-hybrid">Vancouver / Hybrid</SelectItem>
                        <SelectItem value="london-hybrid">London / Hybrid</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>

                {/* Seniority + Target + Employment + Work Mode */}
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-sm font-medium text-foreground mb-1.5 flex items-center gap-2">
                      <Layers className="w-3.5 h-3.5 text-muted-foreground" /> Seniority Level
                    </Label>
                    <Select value={jd.seniority} onValueChange={(v) => set("seniority", v)}>
                      <SelectTrigger className={selectCls}>
                  ...
                  <div>
                    ...
                    <Select value={jd.employmentType} onValueChange={(v) => set("employmentType", v)}>
                      <SelectTrigger className={selectCls}>
                  ...
                  <div>
                    ...
                    <Select value={jd.workMode} onValueChange={(v) => set("workMode", v)}>
                      <SelectTrigger className={selectCls}>
              ...
              <div className="flex items-center justify-between py-2 pb-8">
                ...
                <div className="flex items-center gap-3">
                  <Button
                    variant="outline"
                    onClick={() => { setSaveStatus("saving"); setTimeout(() => setSaveStatus("saved"), 900); }}
                    className="gap-1.5 border-[rgba(15,17,23,0.15)]"
                  >
                  ...
                  <Button
                    onClick={handlePublish}
                    className="gap-2 bg-[#4f46e5] hover:bg-[#4338ca] text-white shadow-sm shadow-[#4f46e5]/20 px-5"
                  >
                    <Globe className="w-4 h-4" />
                    Publish & View Portal
          ...
          {/* ── Preview Panel ────────────────────────────────────────────── */}
          <div className="w-[300px] shrink-0 border-l border-border bg-[#f4f5f7] overflow-y-auto">
