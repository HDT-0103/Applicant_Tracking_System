...
class SupabaseCatalogRepo:
    ...
    def read_analytics(self) -> tuple[list, list, list]:
        jobs = (
            self._client.table("jobs_posting")
            .select(
        ...
        applications = (
            self._client.table("applications")
            .select(
                "id, job_posting_id, referral_source, experience_bucket,"
        ...
        candidates = (
            ...
            .select("uuid, current_location, github_username, linkedin_url")
            .execute()
        ).data or []
