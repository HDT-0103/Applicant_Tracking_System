          ...
          userRole === "hr" && (
            ...
            >
              {reviewStatus?.overall_status === "waiting_for_tls"
                ? "⏳ Waiting for 80% Tech Lead approval before scheduling"
                : reviewStatus?.overall_status === "waiting_for_hr"
                  ? "⚠️ Waiting for HR final approval before scheduling"
                  : reviewStatus?.overall_status === "rejected_by_tls" || reviewStatus?.overall_status === "rejected_by_hr"
...
function ReviewPanel({
  ...
  return (
    <div style={{ marginTop: 24, padding: 24, background: D.canvas, borderRadius: 12, border: `1px solid ${D.line}` }}>
      <h3 style={{ fontSize: 16, fontWeight: 600, color: D.ink, marginBottom: 16 }}>Review Progress</h3>
      
      {!reviewStatus ? (
        <div style={{ fontSize: 13, color: D.muted }}>Loading review status...</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          {/* Tech Lead Progress */}
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, color: D.ink, marginBottom: 12 }}>
              Tech Lead Approvals ({reviewStatus.approved_tls}/{reviewStatus.total_tls})
            </div>
            
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
              {reviewStatus.tl_reviews.map((r, i) => (
                <div key={i} style={{ padding: 12, background: D.surface, borderRadius: 8, border: `1px solid ${D.line}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 13, fontWeight: 500, color: D.ink }}>Tech Lead Reviewer</span>
                    <span style={{ 
                      fontSize: 12, fontWeight: 600, 
                      color: r.decision === "approved" ? D.mint : D.red 
                    }}>
                      {r.decision.toUpperCase()}
                    </span>
                  </div>
                  {r.review_text && (
                    <div style={{ fontSize: 13, color: D.muted, fontStyle: "italic" }}>&ldquo;{r.review_text}&rdquo;</div>
                  )}
                </div>
              ))}
              
              {reviewStatus.tl_reviews.length === 0 && (
                <div style={{ fontSize: 13, color: D.dim }}>No Tech Lead reviews yet.</div>
              )}
            </div>

            {/* If Tech Lead, allow them to vote */}
            {userRole === "tech_lead" && status === "waiting_for_tls" && (
              <div style={{ padding: 16, border: `1px solid ${D.line}`, borderRadius: 8 }}>
                <div style={{ fontSize: 14, fontWeight: 500, color: D.ink, marginBottom: 12 }}>Your Decision</div>
                <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                  <button
                    onClick={() => setDecision("approved")}
                    style={{
                      flex: 1, padding: "10px", borderRadius: 6,
                      background: decision === "approved" ? D.mint : "transparent",
                      color: decision === "approved" ? "#fff" : D.mint,
                      border: `1px solid ${D.mint}`, cursor: "pointer", fontWeight: 500
                    }}
                  >
                    Approve
                  </button>
                  <button
                    ...
                    style={{
                      flex: 1, padding: "10px", borderRadius: 6,
                      background: decision === "rejected" ? D.red : "transparent",
                      color: decision === "rejected" ? "#fff" : D.red,
                      border: `1px solid ${D.red}`, cursor: "pointer", fontWeight: 500
                    }}
                  >
                    Reject
                  </button>
                ...
                <textarea
                  ...
                  placeholder="Leave a comment (required if rejecting)..."
                  style={{
                    width: "100%", padding: 12, borderRadius: 6,
                    border: `1px solid ${D.line}`, background: D.surface, color: D.ink,
                    marginBottom: 16, minHeight: 80, resize: "vertical"
                  }}
                />
                <button
                  onClick={handleSubmit}
                  disabled={!decision || submitting || (decision === "rejected" && !text)}
                  style={{
                    width: "100%", padding: "10px", borderRadius: 6,
                    background: D.blue, color: "#fff", border: "none",
                    cursor: "pointer", fontWeight: 600, opacity: (!decision || submitting) ? 0.6 : 1
                  }}
                >
          ...
          {/* HR Review Section */}
          {userRole === "hr" && (
            <div style={{ borderTop: `1px dashed ${D.line}`, paddingTop: 24 }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: D.ink, marginBottom: 12 }}>
                HR Final Approval
              </div>
              
              {status === "waiting_for_tls" && (
                <div style={{ fontSize: 13, color: D.amber, padding: 12, background: `${D.amber}10`, borderRadius: 6 }}>
                  Waiting for 80% Tech Lead approval before you can review.
                </div>
              )}
              
              {status === "rejected_by_tls" && (
                <div style={{ fontSize: 13, color: D.red, padding: 12, background: `${D.red}10`, borderRadius: 6 }}>
                  Candidate was rejected by &gt;20% Tech Leads.
                </div>
              )}

              {status === "ready_to_schedule" && (
                <div style={{ fontSize: 13, color: D.mint, padding: 12, background: `${D.mint}10`, borderRadius: 6 }}>
                  You have approved this candidate. Ready to schedule.
                </div>
              )}
              ...
              {status === "waiting_for_hr" && (
                <div style={{ padding: 16, border: `1px solid ${D.line}`, borderRadius: 8 }}>
                  <div style={{ fontSize: 13, color: D.dim, marginBottom: 16 }}>
                    Tech Leads have approved. Please review their comments and make a final decision.
                  </div>
                  <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                    <button
                      onClick={() => setDecision("approved")}
